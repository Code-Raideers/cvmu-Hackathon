...
export default function Searching() {
  ...
  const Smash = (e)=> {
    ...
    const filtered = jobs.filter((item) => {
      if (item.login.toLowerCase().includes(jobValue.toLowerCase())&&(item.node_id.toLowerCase().includes(locationValue.toLowerCase())))return item;
    });
  ...
  return (
    ...
    <section className="search">
        ...
        <div className="locationSearch">
          <Search justRef={locationRef} placeholder="where do you want to work" />
        </div>
...
function Search({ placeholder, justRef }) {
  ...
  return (
    <>
      ...
      <input ref={justRef} type="search" placeholder={placeholder} className='searchField' />
    </>
  );
}
